...
import avfLesson from "../../data/questions/avf";

export default function AvfQuizPage() {
  const LESSON_URL = "/docs/arterial/arteriovenous-fistula"; // ✅ slug بتاعك
  ...
  const activeCase = useMemo(() => {
    ...
    return avfLesson.cases.find((c) => c.id === activeCaseId) || null;
  }, [activeCaseId]);
    ...
    <Layout title="AVF Quiz" description="Quiz for Arteriovenous Fistula lesson">
      <main style={{ padding: "24px 0" }}>
        <div className="container">
          ...
          {mode === "menu" ? (
            <>
              <h1 style={{ marginBottom: 6 }}>AVF • Lesson Quiz</h1>
              <div style={{ opacity: 0.75, marginBottom: 18 }}>
                {avfLesson.quiz.questions.length} questions • {avfLesson.cases.length} case discussions
              </div>

              <Card
                title="Quiz"
                subtitle={`${avfLesson.quiz.questions.length} Questions (MCQ)`}
          ...
          {mode === "quiz" ? (
            <StepQuiz
              key="avf-quiz"
              title={avfLesson.quiz.title}
              questions={avfLesson.quiz.questions}
              onExit={() => setMode("menu")}
              autoStart
            />
          ) : null}
