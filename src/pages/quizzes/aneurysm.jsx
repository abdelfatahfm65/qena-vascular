...
import aneurysmLesson from "../../data/questions/aneurysm";

export default function AneurysmQuizPage() {
  const LESSON_URL = "/docs/arterial/arterial-aneurysms"; // ✅ slug بتاعك
  ...
  const activeCase = useMemo(() => {
    ...
    return aneurysmLesson.cases.find((c) => c.id === activeCaseId) || null;
  }, [activeCaseId]);
    ...
    <Layout title="Arterial Aneurysms Quiz" description="Quiz for Arterial Aneurysms lesson">
      <main style={{ padding: "24px 0" }}>
        <div className="container">
          ...
          {mode === "menu" ? (
            <>
              <h1 style={{ marginBottom: 6 }}>Arterial Aneurysms • Lesson Quiz</h1>
              <div style={{ opacity: 0.75, marginBottom: 18 }}>
                {aneurysmLesson.quiz.questions.length} questions • {aneurysmLesson.cases.length} case discussions
              </div>

              <Card
                title="Quiz"
                subtitle={`${aneurysmLesson.quiz.questions.length} Questions (MCQ)`}
          ...
          {mode === "quiz" ? (
            <StepQuiz
              key="aneurysm-quiz"
              title={aneurysmLesson.quiz.title}
              questions={aneurysmLesson.quiz.questions}
              onExit={() => setMode("menu")}
              autoStart
            />
          ) : null}
