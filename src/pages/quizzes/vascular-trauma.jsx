...
import vascularTraumaLesson from "../../data/questions/vascular-trauma";


export default function VascularTraumaQuizPage() {
  const LESSON_URL = "/docs/arterial/vascular-trauma"; // ✅ slug بتاعك
  ...
  const activeCase = useMemo(() => {
    ...
    return vascularTraumaLesson.cases.find((c) => c.id === activeCaseId) || null;
  }, [activeCaseId]);
    ...
    <Layout title="Vascular Trauma Quiz" description="Quiz for Vascular Trauma lesson">
      <main style={{ padding: "24px 0" }}>
        <div className="container">
          ...
          {mode === "menu" ? (
            <>
              <h1 style={{ marginBottom: 6 }}>Vascular Trauma • Lesson Quiz</h1>
              <div style={{ opacity: 0.75, marginBottom: 18 }}>
                {vascularTraumaLesson.quiz.questions.length} questions • {vascularTraumaLesson.cases.length} case discussions
              </div>

              <Card
                title="Quiz"
                subtitle={`${vascularTraumaLesson.quiz.questions.length} Questions (MCQ)`}
              ...
              {vascularTraumaLesson.cases.map((c) => (
                <Card
                  key={c.id}
                  title={c.title}
          ...
          {mode === "quiz" ? (
            <StepQuiz
              key="vt-quiz"
              title={vascularTraumaLesson.quiz.title}
              questions={vascularTraumaLesson.quiz.questions}
              onExit={() => setMode("menu")}
              autoStart
            />
          ) : null}
