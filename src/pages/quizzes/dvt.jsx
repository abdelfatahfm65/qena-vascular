...
import dvtLesson from "../../data/questions/dvt";
...
export default function DvtQuizPage() {
  ...
  const activeCase = useMemo(() => {
    ...
    return dvtLesson.cases.find((c) => c.id === activeCaseId) || null;
  }, [activeCaseId]);
    ...
    <Layout title="DVT Quiz" description="Quiz for Deep Vein Thrombosis lesson">
      <main style={{ padding: "24px 0" }}>
        <div className="container">
          ...
          {mode === "menu" ? (
            <>
              <h1 style={{ marginBottom: 6 }}>DVT • Lesson Quiz</h1>
              <div style={{ opacity: 0.75, marginBottom: 18 }}>
                {dvtLesson.quiz.questions.length} questions • {dvtLesson.cases.length} case discussions
              </div>

              <Card
                title="Quiz"
                subtitle={`${dvtLesson.quiz.questions.length} Questions (MCQ)`}
          ...
          {mode === "quiz" ? (
            <StepQuiz
              key="dvt-quiz"
              title={dvtLesson.quiz.title}
              questions={dvtLesson.quiz.questions}
              onExit={() => setMode("menu")}
              autoStart
            />
          ) : null}
