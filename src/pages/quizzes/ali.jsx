...
import aliLesson from "../../data/questions/ali";

export default function AliQuizPage() {
  const LESSON_URL = "/docs/arterial/acute-limb-ischemia"; // ✅ slug بتاعك
  ...
  const activeCase = useMemo(() => {
    ...
    return aliLesson.cases.find((c) => c.id === activeCaseId) || null;
  }, [activeCaseId]);
    ...
    <Layout title="ALI Quiz" description="Quiz for Acute Limb Ischemia lesson">
      <main style={{ padding: "24px 0" }}>
        <div className="container">
          ...
          {mode === "menu" ? (
            <>
              <h1 style={{ marginBottom: 6 }}>ALI • Lesson Quiz</h1>
              <div style={{ opacity: 0.75, marginBottom: 18 }}>
                {aliLesson.quiz.questions.length} questions • {aliLesson.cases.length} case discussions
              </div>

              <Card
                title="Quiz"
                subtitle={`${aliLesson.quiz.questions.length} Questions (MCQ)`}
          ...
          {mode === "quiz" ? (
            <StepQuiz
              key="ali-quiz"
              title={aliLesson.quiz.title}
              questions={aliLesson.quiz.questions}
              onExit={() => setMode("menu")}
              autoStart
            />
          ) : null}
