...
import varicoseVeinsLesson from "../../data/questions/varicoseVeins";
...
export default function VaricoseVeinsQuizPage() {
  ...
  const activeCase = useMemo(() => {
    ...
    return varicoseVeinsLesson.cases.find((c) => c.id === activeCaseId) || null;
  }, [activeCaseId]);
    ...
    <Layout title="Varicose Veins Quiz" description="Quiz for Varicose Veins lesson">
      <main style={{ padding: "24px 0" }}>
        <div className="container">
          ...
          {mode === "menu" ? (
            <>
              <h1 style={{ marginBottom: 6 }}>VV • Lesson Quiz</h1>
              <div style={{ opacity: 0.75, marginBottom: 18 }}>
                {varicoseVeinsLesson.quiz.questions.length} questions • 2 case discussions
              </div>

              <Card
                title="Quiz"
                subtitle={`${varicoseVeinsLesson.quiz.questions.length} Questions (MCQ)`}
              ...
              {varicoseVeinsLesson.cases.map((c) => (
                <Card
                  key={c.id}
                  title={c.title}
          ...
          {mode === "quiz" ? (
            <StepQuiz
              key="vv-quiz"
              title={varicoseVeinsLesson.quiz.title}
              questions={varicoseVeinsLesson.quiz.questions}
              onExit={() => setMode("menu")}
              autoStart // ✅ NEW: يخش مباشرة على الأسئلة
            />
          ) : null}
            ...
            <>
              ...
              >
                ...
                <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <SecondaryBtn onClick={() => setMode("menu")}>Back to menu</SecondaryBtn>
